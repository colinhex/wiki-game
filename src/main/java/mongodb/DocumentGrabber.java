...
public class DocumentGrabber {
    ...
    private static final String COLLECTION_NAME = "gpt_bt";
    ...
    private FindIterable<Document> getDocuments() {
        return getMongoCollection().find().batchSize( 5 );
    }


    public List<WikiGame> getDocumentStream() {
        FindIterable<Document> documents = getDocuments();
        final LinkedList<WikiGame> wikiGames = new LinkedList<>();
        documents.forEach( document -> {
            try {
                wikiGames.add( objectMapper.readValue( document.toJson(), WikiGame.class ) );
